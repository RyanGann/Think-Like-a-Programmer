...
int main() {
	...
	enum modeType {UPPERCASE, LOWERCASE, PUNCTUATION}; // Declare enum mode types for more readable code when modes are switched
	modeType mode = UPPERCASE;  // Start with UPPERCASE mode, going in the order of uppercase->lowecase->punctution and then back to uppercase to repeat the cycle
	char digitChar;
	do {
		digitChar = cin.get();
		int number = (digitChar - '0'); //Convert to integer
		digitChar = cin.get();
		while ((digitChar != 10) && (digitChar != ',')) {  // This loop allows for the input of any size integer
		...
		switch (mode) {
			case UPPERCASE: 
				number = number % 27;
				outputCharacter = number + 'A' - 1;   // Decode the number and find the character it corresponds to 
				if (number == 0) {
					mode = LOWERCASE;
					continue;
				}
